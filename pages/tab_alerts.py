...
def render(build_stock_rows_cached) -> None:
    ...
    alert_email_input = st.text_input(
        ...
        value=stored_email, placeholder="you@example.com", key="alert_email_input",
    )
    ...
    if alert_email and smtp_configured():
        if st.button("\U0001f4e7 Send Test Email", use_container_width=False):
            try:
                ok, err = send_email(
                    alert_email, "Nifty50 Alert - Test Email",
                ...
                if ok:
                    ...
                    log.warning("tab_alerts: send_email error: %s", err)
    ...
    with r2c1:
        al_stock_name = st.selectbox(
            "Stock", [s["name"] for s in NIFTY50], key="al_stock"
        )
    with r2c2:
        al_direction = st.selectbox(
            "Alert when price", ["rises above \u2191", "drops below \u2193"], key="al_dir"
        )
    ...
    if _px_key not in st.session_state:
        live_px = 100.0
        # ── Spinner: live price fetch for alert stock ────────────────────────
        with st.spinner(f"Fetching current price for {al_stock_name}…"):
            try:
                ...
                if not live_data.empty and "Close" in live_data.columns:
                    ...
                    log.warning("tab_alerts: empty data for '%s'", al_sym)
            except OSError as exc:
                log.error("tab_alerts: network error for '%s': %s", al_sym, exc, exc_info=True)
                st.warning(f"Could not fetch live price for {al_stock_name}; defaulting to Rs.\u00a0100.")
            except Exception as exc:  # noqa: BLE001
                log.error("tab_alerts: unexpected for '%s': %s", al_sym, exc, exc_info=True)
    ...
    with st.form("add_alert_form", clear_on_submit=True):
        al_thresh = st.number_input(
            "Target Price (Rs.)", min_value=0.01, value=live_px, step=1.0
        )
        submitted = st.form_submit_button(
            "Set Alert", type="primary", use_container_width=True
        )
        if submitted:
            if al_thresh <= 0:
                ...
                try:
                    add_alert(
                        stock=al_stock_name, symbol=al_sym, direction=direction,
    ...
    if not active:
        ...
        for al in active:
            ...
            ) or (al["direction"] == "below" and live_px_al <= al["threshold"])
            ...
            with col_del:
                if st.button("\U0001f5d1\ufe0f", key=f"del_{al['id']}", help="Remove"):
                    try:
                        ...
                        log.error("tab_alerts: remove_alert id=%s failed: %s",
                                  al["id"], exc, exc_info=True)
