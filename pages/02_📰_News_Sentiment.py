...
# Guard: only call set_page_config when run as standalone page
try:
...
<div style="display:flex;align-items:center;gap:1rem;margin-bottom:.5rem;">
  ...
  <div>
    <div style="font-size:1.7rem;font-weight:700;">News Sentiment</div>
    <div style="font-size:.85rem;color:#64748b;margin:0;">Analyse market mood from recent headlines</div>
  </div>
</div>
""", unsafe_allow_html=True)
st.markdown("<span style='background:#10b981;color:#fff;padding:2px 10px;border-radius:99px;font-size:.75rem;'>LIVE</span>&nbsp; TextBlob NLP sentiment analysis", unsafe_allow_html=True)
...
stock  = st.selectbox("🏦 Select Company", NAMES, key="news_stock")
...
if not news:
    ...
    for item in news[:20]:
        try:
            # yfinance news items may use 'title' or nested 'content'
            title = (
                item.get("title")
    ...
    if rows:
        ...
        pos = len(df_news[df_news["Sentiment"] == "🟢 Positive"])
        neg = len(df_news[df_news["Sentiment"] == "🔴 Negative"])
        neu = len(df_news[df_news["Sentiment"] == "⚪ Neutral"])
        ...
        if pos + neg + neu > 0:
            try:
                ...
                st.warning(f"⚠️ {e}")
        st.dataframe(df_news[["Headline", "Sentiment", "Polarity", "Subjectivity"]],
