...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    """Build RSI / MACD / BB / EMA-cross features."""
    ...
    feat["bb_pos"] = (c - ma20) / std20.replace(0, np.nan)  # z-score inside band
...
def make_labels(feat: pd.DataFrame, threshold: float = 0.015) -> pd.Series:
    """Label based on 5-day forward return: +1 BUY, -1 SELL, 0 HOLD."""
...
def train_and_signal(feat: pd.DataFrame):
    """Train RF + GB, return latest signal, confidence, history."""
...
if hist.empty or len(hist) < 60:
    ...
    with tab1:
        if len(hist_sigs) > 0:
            ...
            # Scatter BUY / SELL dots on price line
...
if scan_mode:
    ...
    st.dataframe(
        scan_df[display_cols].style.background_gradient(
