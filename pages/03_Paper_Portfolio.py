...
with st.form("pp_add", clear_on_submit=True):
    ...
    with c3: buy_p = st.number_input("Buy Price (₹)", min_value=0.01, value=100.0, step=1.0)
...
if not st.session_state.pp_holdings:
    ...
    with st.spinner("Fetching live prices…"):
        for i, h in enumerate(st.session_state.pp_holdings):
            ...
            try:
                ...
                if not tick.empty: live = float(tick["Close"].iloc[-1])
            ...
            total_inv += inv_val
            total_cur += cur_val
            rows.append({"#": i, "Stock": h["stock"], "Qty": h["qty"],
                "Buy (₹)": round(h["buy_price"], 2), "Live (₹)": round(live, 2),
                "Invested (₹)": round(inv_val, 2), "Current (₹)": round(cur_val, 2),
                "P&L (₹)": round(pl, 2), "Return (%)": round(pct, 2),
                "Added": h["added"], "Note": h.get("note", ""),
            })
    ...
    with c1:
        try:
            fig_pl = px.bar(df, x="Stock", y="P&L (₹)", color="P&L (₹)",
                color_continuous_scale=["#ef4444","#f59e0b","#10b981"],
                color_continuous_midpoint=0, title="P&L per Stock", height=320, text="P&L (₹)")
    ...
    with c2:
        try:
            fig_pie = px.pie(df, names="Stock", values="Current (₹)",
    ...
    if st.button("🗑️ Clear All Holdings", key="pp_clear"):
        st.session_state.pp_holdings = []; st.rerun()
