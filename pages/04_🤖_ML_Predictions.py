...
st.set_page_config(
    page_title="ML Predictions — NSE Tracker",
    page_icon="🤖",
    layout="wide",
...
try:
    ...
    pass
st.sidebar.markdown("<h3 style='color:#fff;margin:0 0 .4rem 0;font-size:1rem;'>ML Predictions</h3>",
                    unsafe_allow_html=True)
if user:
...
try:
    ...
    with tab1:
        ...
        if len(fut_dates) >= 2:
            try:
                fig_dd.add_vrect(
                    x0=str(fut_dates[0].date()), x1=str(fut_dates[-1].date()),
    ...
    with tab2:
        ...
        fig_pr.add_hline(y=trigger, line_dash="dash", line_color="#ef4444",
                         annotation_text=f"Trigger ₹{trigger:,.0f}",
                         annotation_position="bottom right")
        if len(fut_dates) >= 2:
            try:
                fig_pr.add_vrect(
