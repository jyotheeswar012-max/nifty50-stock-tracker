...
st.caption("Your watchlists are persisted to disk (SQLite) and survive page refreshes.")

# ---------------------------------------------------------------------------
# Ephemeral filesystem warning (Streamlit Cloud)
# ---------------------------------------------------------------------------
# On Streamlit Cloud the container filesystem resets on every redeploy, which
# silently wipes the SQLite watchlist DB.  Warn users so they export first.
if os.environ.get("STREAMLIT_SHARING_MODE") or os.environ.get("IS_STREAMLIT_CLOUD"):
    st.warning(
        ...
        "watchlist data stored in SQLite **will be erased on every redeploy**. "
        "Use the **Export as CSV** button below to back up your lists before redeploying.",
        icon="⚠️",
    )
