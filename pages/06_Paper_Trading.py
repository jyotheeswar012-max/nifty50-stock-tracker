...
if is_guest(): login_nudge("save your paper trading progress")
...
pnl=total_eq-STARTING_CAPITAL; pnl_pct=pnl/STARTING_CAPITAL*100
...
with col_reset:
    if st.button("🔄 Reset Account",key="pt_reset"):
        ...
        st.success("✅ Account reset to ₹10,00,000"); st.rerun()
with col_pdf:
    if st.button("📄 PDF Report",type="primary",use_container_width=True,key="pt_pdf"):
        try:
            ...
            pdf.cell(0,8,f"User: {name}",ln=True,align="C"); pdf.ln(4)
            pdf.set_font("Helvetica","B",13); pdf.cell(0,10,"Account Summary",ln=True)
            pdf.set_font("Helvetica","",11)
            for lbl,val in [("Starting Capital",f"Rs.{STARTING_CAPITAL:,.2f}"),("Cash Balance",f"Rs.{st.session_state.pt_balance:,.2f}"),("Portfolio Value",f"Rs.{port_val:,.2f}"),("Total Equity",f"Rs.{total_eq:,.2f}"),("Net P&L",f"Rs.{pnl:+,.2f}"),("Total Trades",str(len(st.session_state.pt_trades)))]:
                pdf.cell(60,8,lbl+":",border=0); pdf.cell(0,8,val,ln=True)
            pdf_bytes=bytes(pdf.output())
            st.download_button("⬇️ Download PDF",data=pdf_bytes,file_name=f"paper_trading_{datetime.now(IST_TZ).strftime('%Y%m%d_%H%M')}.pdf",mime="application/pdf",key="pt_dl")
        except Exception as e:
            st.warning(f"⚠️ PDF error: {e}")

st.markdown('<hr class="ui-divider">',unsafe_allow_html=True)
tab_buy,tab_sell,tab_port,tab_log,tab_chart=st.tabs(["🟢  Buy","🔴  Sell","💼  Portfolio","📜  Trade Log","📈  Equity Curve"])

with tab_buy:
    st.markdown("<p class='sec-label'>🟢 Place a Buy Order</p>",unsafe_allow_html=True)
    bc1,bc2,bc3=st.columns(3)
    with bc1: buy_stock=st.selectbox("Stock",NIFTY50_NAMES,key="pt_buy_stock")
    ...
    with bc3:
        ...
        if buy_price and buy_price>0: st.metric("Live Price",f"₹{buy_price:,.2f}")
        else: st.warning("⏳ Price unavailable"); buy_price=0.0
    ...
    if st.button("🟢 Execute Buy",type="primary",key="pt_exec_buy"):
        if buy_price<=0: st.error("❌ Cannot fetch live price.")
        elif buy_cost>st.session_state.pt_balance: st.error(f"❌ Insufficient balance. Need ₹{buy_cost:,.2f}")
        else:
            hh=st.session_state.pt_holdings
            if buy_sym in hh:
                old=hh[buy_sym]; tq=old["qty"]+buy_qty; avg=(old["avg_price"]*old["qty"]+buy_price*buy_qty)/tq
                hh[buy_sym]={"qty":tq,"avg_price":round(avg,4),"name":buy_stock}
            else: hh[buy_sym]={"qty":buy_qty,"avg_price":round(buy_price,4),"name":buy_stock}
            st.session_state.pt_balance-=buy_cost
            st.session_state.pt_trades.append({"time":datetime.now(IST_TZ).strftime("%H:%M:%S"),"stock":buy_stock,"type":"BUY","qty":buy_qty,"price":round(buy_price,2),"value":round(buy_cost,2)})
            _snapshot(); st.success(f"✅ Bought {buy_qty}×{buy_stock} @ ₹{buy_price:,.2f}"); st.rerun()
...
with tab_sell:
    ...
    if not st.session_state.pt_holdings:
        ...
        with sc2: sell_qty=st.number_input("Quantity",min_value=1,max_value=held_qty,value=1,step=1,key="pt_sell_qty")
        with sc3:
            sell_price=_live_price(sell_sym)
            if sell_price and sell_price>0: st.metric("Live Price",f"₹{sell_price:,.2f}")
            else: st.warning("⏳ Price unavailable"); sell_price=0.0
        avg_pr=st.session_state.pt_holdings[sell_sym]["avg_price"]
        sell_val=_sf(sell_price)*int(sell_qty); est_pnl=(_sf(sell_price)-avg_pr)*sell_qty
        ...
        if st.button("🔴 Execute Sell",type="primary",key="pt_exec_sell"):
            if sell_price<=0: st.error("❌ Cannot fetch live price.")
            else:
                hh=st.session_state.pt_holdings
                if sell_qty>=held_qty: del hh[sell_sym]
                else: hh[sell_sym]["qty"]-=sell_qty
                st.session_state.pt_balance+=sell_val
                st.session_state.pt_trades.append({"time":datetime.now(IST_TZ).strftime("%H:%M:%S"),"stock":sell_name,"type":"SELL","qty":sell_qty,"price":round(sell_price,2),"value":round(sell_val,2)})
                _snapshot(); st.success(f"✅ Sold {sell_qty}×{sell_name} @ ₹{sell_price:,.2f} | P&L: ₹{est_pnl:+,.2f}"); st.rerun()
...
with tab_log:
    ...
    if not st.session_state.pt_trades: st.info("💡 No trades yet.")
    else: st.dataframe(pd.DataFrame(st.session_state.pt_trades[::-1]),use_container_width=True,hide_index=True)

with tab_chart:
    st.markdown("<p class='sec-label'>📈 Equity Curve</p>",unsafe_allow_html=True)
    eq=st.session_state.pt_equity
    if len(eq)<2: st.info("💡 Execute at least 2 trades to see the equity curve.")
    else:
        try:
            eq_df=pd.DataFrame(eq)
            fig_eq=go.Figure()
            fig_eq.add_trace(go.Scatter(x=eq_df["time"],y=eq_df["equity"],mode="lines+markers",fill="tozeroy",name="Equity",line=dict(color="#6366f1",width=2.5),fillcolor="rgba(99,102,241,0.1)",marker=dict(size=6,color="#6366f1")))
            fig_eq.add_hline(y=STARTING_CAPITAL,line_dash="dash",line_color="#f59e0b",annotation_text=f"Start ₹{STARTING_CAPITAL:,.0f}")
            fig_eq.update_layout(**PLT_LAYOUT,height=400,xaxis_title="Time",yaxis_title="Equity (₹)")
            style_fig(fig_eq); st.plotly_chart(fig_eq,use_container_width=True)
