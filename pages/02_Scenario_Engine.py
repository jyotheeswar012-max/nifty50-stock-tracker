...
st.set_page_config(page_title="Scenario Engine", page_icon="🧪", layout="wide", initial_sidebar_state="collapsed")
...
MACRO_EVENTS = {
    ...
    "🟢 Union Budget — pro-growth":{"desc": "Capital expenditure boost",   "sectors": {"Construction": +5.0, "Financial Services": +2.0}},
...
for i, row in enumerate(st.session_state.se_portfolio):
    ...
    with c1:
        st.session_state.se_portfolio[i]["stock"] = st.selectbox(
            f"Stock #{i+1}", names,
            index=names.index(row["stock"]) if row["stock"] in names else 0,
    ...
    with c3:
        st.session_state.se_portfolio[i]["buy_price"] = st.number_input(
            ...
            key=f"se_bp{i}", label_visibility="collapsed"
        )
    with c4:
...
if st.button("🚀 Run Scenario", type="primary", key="se_run"):
    with st.spinner("Fetching live prices & running simulation…"):
        ...
        for row in st.session_state.se_portfolio:
            ...
            stock_move = custom_move * beta if custom_move != 0 else sector_move * beta
    ...
    if results:
        ...
        m1.metric("Before", f"₹{total_old:,.2f}")
        m2.metric("After",  f"₹{total_new:,.2f}")
        m3.metric("P&L",    f"₹{total_pl:+,.2f}")
        m4.metric("Return", f"{total_pct:+.2f}%")
        if total_pl > 0:
            st.success(f"✅ Gain: ₹{total_pl:,.2f} ({total_pct:+.2f}%)")
        elif total_pl < 0:
            st.error(f"❌ Loss: ₹{abs(total_pl):,.2f} ({total_pct:.2f}%)")
        ...
        try:
            fig = px.bar(
                ...
                text="P&L (₹)",
                title=f"P&L per Stock — {event_name}",
                height=380,
            )
