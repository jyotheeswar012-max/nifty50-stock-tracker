...
if st.button("🚀 Run Scenario", type="primary", key="se_run"):
        ...
        for row in st.session_state.se_portfolio:
            ...
            if live_price <= 0:
                try:
                    ...
                    if not h.empty: live_price = float(h["Close"].iloc[-1])
                except Exception:
                    live_price = 100.0
            sector = meta["sector"]
            beta = meta["beta"]
    ...
    if results:
        ...
        if total_pl > 0: st.success(f"✅ Gain: ₹{total_pl:,.2f} ({total_pct:+.2f}%)")
        elif total_pl < 0: st.error(f"❌ Loss: ₹{abs(total_pl):,.2f} ({total_pct:.2f}%)")
        df = pd.DataFrame(results)
        st.dataframe(df, use_container_width=True, hide_index=True)
        try:
            fig = px.bar(df, x="Stock", y="P&L (₹)", color="P&L (₹)",
