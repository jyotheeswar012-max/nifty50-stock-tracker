...
def _render_heatmap(syms: list[str], names: list[str], title: str) -> None:
    ...
    fig = go.Figure(go.Heatmap(
        z=corr.tolist(), x=lbls, y=lbls, text=text,
...
def _render_sparklines(syms: list[str], names: list[str]) -> None:
    ...
    fig = make_subplots(rows=ROWS, cols=COLS, horizontal_spacing=0.04, vertical_spacing=0.08)
    ...
    for idx, (sym, lbl, vals) in enumerate(valid):
        ...
        fig.add_trace(
            go.Scatter(x=list(range(len(vals))), y=vals, mode="lines",
                       line=dict(color=colour, width=1.5), name=lbl, showlegend=False,
        ...
        fig.add_annotation(
            ...
            x=(col - 0.5) / COLS, y=1.0 - (row - 1) / ROWS + 0.005,
            showarrow=False, font=dict(size=9, color=colour), xanchor="center",
        )
    ...
    fig.update_layout(
        ...
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        font=dict(color="#e2e8f0"), margin=dict(t=80, b=10, l=10, r=10),
    )
    st.plotly_chart(fig, use_container_width=True)
...
def render(
    ...
    _render_heatmap(sector_syms, sector_names, f"30-Day Correlation \u2014 {sel_sec}")
    divider()
